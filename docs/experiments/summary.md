...
本文档汇总当前项目已经完成的主要实验，帮助快速判断系统目前处在什么阶段、哪些策略有效、Demo 使用哪套正式配置。
...
结论：前三周主线和第四周收尾材料已经形成闭环。当前可用 `routed_v17` 检索路线和默认生成 prompt 做最终 Demo。
...
3. summary 的 `HitAll@8` 仍然偏低，生成 correct rate 只有 50.00%，是后续继续优化的主要对象。
...
- 检索：`17_routed_compare_raw_entity_slots`。
- 生成：默认 prompt 约束。
- 展示：问题、路由策略、召回证据、引用来源、最终回答。

后续优化建议：

1. summary：继续优化多文档覆盖和答案组织，但 `summary_evidence_pack` 暂不作为正式链路。
2. fact：减少证据已命中时的错误拒答，重点处理表格字段读取。
3. 工程化：如果数据量扩大，再考虑 Milvus、缓存、异步生成和服务化部署。
