...
|---|---:|---:|---:|---:|---:|---:|
| routed | 93.65% | 95.10% | 97.50% | 97.50% | 89.17% | 92.50% |
...
- 本实验使用 `compare_financial_table_no_unit_rule.jsonl`，将“预计 2026 年营收/营业收入”类问题改写为 `2026E 营收 营业收入 主营业务收入`，不在检索 query 中加入“（百万元）”。
- 当前改写缓存由 `--fallback-without-api` 生成，用于验证检索表述方向；正式接入 API 改写时仍应沿用“不带单位”的约束。
...
- `compare_007` compare coverage=1/2
  ...
  top_sources: ['data/raw/healthcare/PRS.pdf', 'data/raw/healthcare/PRYK.pdf', 'data/raw/healthcare/BAST.pdf', 'data/raw/healthcare/HXYK.pdf']
- `compare_008` compare coverage=0/2
  query: 昭衍新药 vs 通策医疗，预计哪家公司在2026年的营收更高
  top_sources: ['data/raw/healthcare/YKSW.pdf', 'data/raw/healthcare/BAST.pdf', 'data/raw/healthcare/SYXY.pdf', 'data/raw/healthcare/GJYX.pdf']
