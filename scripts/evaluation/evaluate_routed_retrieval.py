...
from financial_report_rag.retrieval.parent_document import ParentDocumentStore, SearchLikeResult  # noqa: E402
from financial_report_rag.retrieval.reranker import CrossEncoderReranker, RerankConfig, RerankResult  # noqa: E402
from financial_report_rag.retrieval.routed_retriever import RoutedRetrievalConfig, route_results  # noqa: E402
...
def run_compare_rerank(
    ...
    return {
        sample.question_id: reranker.rerank(
            sample.query,
            hybrid_results.get(sample.question_id, [])[: config.compare_candidate_top_k],
...
def run_routed_searches(
    ...
    return {
        sample.question_id: route_results(
...
def render_report(
    ...
    lines = [
        ...
        f"- compare 策略：hybrid_top{config.compare_candidate_top_k} + rerank_top{config.compare_top_k} + parent_window",
...
def main() -> None:
    ...
    bm25_results = run_bm25_searches(samples, bm25_path, top_k=candidate_top_k)
    ...
    routed_results = run_routed_searches(samples, hybrid_results, compare_rerank_results, parent_store, config)

    all_scores = {
        "routed": eval_base.score_scheme(samples, routed_results, args.ks, match_level=args.match_level)
    }
    strategy_by_id = {
        sample.question_id: strategy_name(sample.question_type, config)
        for sample in samples
    }
    ...
    write_details(details_path, all_scores, routed_results, strategy_by_id)
